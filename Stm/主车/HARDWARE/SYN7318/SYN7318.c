...
unsigned char my[10] = {0xFD,0x00,0x07,0x1e,0x02 ,0x27,0x10,0x17,0x70,0x05};
int x;

unsigned char Stop_Three_One_ASR_Buf[] = {0xFD,0x00,0x01,0x16};  // 停止三合一识别
...
//	Frame[2] = 0x07;    //由于不确定数据区的长度，所以选择暂时屏蔽
	...
	else                 //有提示音
	{
		...
		Length = strlen((const char *)Pst); //文本或内置MP3提示音的长度
		Frame[1] = 0x00;
		Frame[2] = Length + 5;
		for(i = 0; i < Length; i++)
		{
...
void SYN7318_Test_( void)  // 开启语音测试
{ 
				u8 commom[8]={0x55,0x06,0x20,0x01,0x00,0x00,0x21,0xbb};
				u8 commom_[8]={0x55,0x06,0x10,0x01,0x00,0x00,0x11,0xbb};
				x = 0;
			  Ysn7813_flag =1;
			  Ysn7813_flag_cs =1;
...
//				SYN7318_Put_String(Wake_Up,5);//发送唤醒指令
//				SYN7318_Get_String(Back,4);   //接收反馈信息
				send_data_zigbee( commom_ , 8);		
//				SYN7318_Get_String(Back,4); 	//接收
				delay_ms(1000);
				delay_ms(1000);
				while(x++<4)		//识别三次
				{
					...
					SYN7318_Put_String(my,10);
					SYN7318_Get_String(Back,4);     
					send_data_zigbee( commom , 8);
