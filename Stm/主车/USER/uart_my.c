...
void Normal_data(void)	  // 正常接收8字节控制指令
{
	...
	{									  
		...
		{
			if( (last_code[1]!=Wifi_Rx_Buf[1]) ||
				(last_code[2]!=Wifi_Rx_Buf[2]) ||
				(last_code[3]!=Wifi_Rx_Buf[3]) ||
				(last_code[4]!=Wifi_Rx_Buf[4])||
				(last_code[5]!=Wifi_Rx_Buf[5])	)	//判断是否与上次接受的指令不同
			{
				Rx_Flag=1;OK_flag=0;
				last_code[1]=Wifi_Rx_Buf[1]; 
				last_code[2]=Wifi_Rx_Buf[2];
				last_code[3]=Wifi_Rx_Buf[3];
				last_code[4]=Wifi_Rx_Buf[4];
				last_code[5]=Wifi_Rx_Buf[5];	
			}
			else if(Wifi_Rx_Buf[2]==0x80)			//相同且为状态指令，返回状态数据
				back_state();
			else if(Wifi_Rx_Buf[2]==0x81)			//相同且为rfid状态指令，返回rfid状态数据
				RFID_state();
			else if(Wifi_Rx_Buf[2]==0x82||Wifi_Rx_Buf[2]==0x83)			//相同且为普通常规状态指令，返回按要求的状态
				data_other(0);
			else 									//否则返回应答
				back_function();
		}
					...
					if(sum==Wifi_Rx_Buf[i+6])	 // 判断求和
					{
						...
						if(sum == Wifi_Rx_Buf[6])
						{
								...
								(last_code[5]!=Wifi_Rx_Buf[5])	)	//判断是否与上次接受的指令不同
							{
								...
								RFID_state();
							else if(Wifi_Rx_Buf[2]==0x82)			//相同且为普通常规状态指令，返回按要求的状态
								data_other(0);
							else 									//否则返回应答
								back_function();
						}
