...
void Send_data(unsigned char ch)
{
  ...
  USART1->DR = (ch & 0x1FF);
	while((USART1->SR & USART_FLAG_TC)==SET);
}
...
short Rece_data(unsigned char *ch, unsigned int WaitTime)
{
	...
	int i=WaitTime;
   while (!(USART1->SR & USART_FLAG_RXNE)&&i)//接收缓存空且延时时间末到	    
  		 i--;  
    if(i)
		{
			*ch = ((int)(USART1->DR & 0x1FF)); 
...
{  
    unsigned char EchoByte=0xff;
	int i=2;
//	char c;	  
    short status=STATUS_IO_TIMEOUT;
